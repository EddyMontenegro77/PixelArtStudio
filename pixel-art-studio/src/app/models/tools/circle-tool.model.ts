...
import { getCircleOutlinePoints } from './circle.utils';

export class CircleTool implements Tool {
  name: string = 'Circle';
  icon: string = '/icons/circle_icon_black.svg';
  ...
  onPointerUp(event: PointerEvent, context: ToolContext): boolean {
    ...
    const radius = this.getRadius(this.centerX, this.centerY, endX, endY);
    const points = getCircleOutlinePoints(this.centerX, this.centerY, radius);
    const changed = this.paintCirclePoints(points, context, brushSize, fillColor);
  ...
  renderOverlay(ctx: CanvasRenderingContext2D, context: ToolContext): void {
    ...
    const radius = this.getRadius(this.centerX, this.centerY, this.currentX, this.currentY);
    const points = getCircleOutlinePoints(this.centerX, this.centerY, radius);
  ...
  private getRadius(centerX: number, centerY: number, edgeX: number, edgeY: number): number {
